...
final class Memcache implements CacheHandlerInterface
{
    ...
    {
        extension_loaded('memcache') || $this->exception('require memcache support');
    ...
    {
        false === $this->memcache->set($key, $value, 0, $expire) && $this->exception($key.'缓存值无法设置');
        // 如果限制了最大储存数, 调用队列
    ...
    {
        false === $this->memcache->delete($key) && $this->exception($key.'缓存值无法删除');
    }
    // 清空缓存
    public function clear() : void
    {
        false === $this->memcache->flush() && $this->exception($key.'缓存值无法清空');
    }
