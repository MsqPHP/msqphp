...
class Framework
{
    ...
    public static function init(array $path_config)
    {
        ...
        //安全检测
        require __DIR__.'/Base/Safe/Safe.php';

        try {
            Base\Safe\Safe::check();
        } catch(Base\Safe\SafeException $e) {
    ...
    {
        //加载缓存类
        require __DIR__.'/Base/Cache/Cache.php';
        //加载路由解析类
        require __DIR__.'/Base/Route/Route.php';
        $route_config = require static::$config_path.'route.php';
        //得到路由信息
        $route_info = Base\Route\Route::getRouteInfo($route_config);
        //赋值给$_GET
        $_GET = $route_info['get'];
        define('URL',$route_info['url']);
        //加载app类
        require __DIR__.'/Base/App/App.php';
        $app_config = require static::$config_path.'app.php';
        //初始化APP
        Base\App\App::init($route_info['app'],$app_config);
    }
    /**
     * 框架运行
     * @return void
