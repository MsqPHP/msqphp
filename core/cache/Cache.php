...
final class Cache
{
     ...
     * @return void
     */
    private function setHandler(string $handler = '', array $config = [])
    {
        //为空取默认
        $handler = $handler ?: $this->config['default_handler'];
        //如果不在支持列表中
        if (!in_array($handler, $this->config['sports'])) {
            throw new CacheException($handler . ' 缓存处理器不支持');
        }

        if (empty($config)) {
            $config      = $this->config['handlers_config'][$handler];
            $handler_key = $handler;
        } else {
            $config      = array_merge($this->config['handlers_config'][$handler], $config);
            $handler_key = md5(serialize($config).$handler);
        }

        $this->handlers[$handler_key] = $this->pointer['handler'] = $this->handlers[$handler_key] ?? $this->initHandler($handler, $config);
    }
    /**
     * 加载并返回处理类
     * @param  string $handler 处理类类型
     ...
     * @return 处理类
     */
    private function initHandler(string $handler, array $config) : handlers\CacheHandlerInterface
    {
        static $files  = [];
