...
abstract class View
{
    ...
    public function assign($tpl_var, $value='', bool $cache=false, bool $type=false) {
        ...
        //转义
        $type === true && ($value = \Core\Vendor\Filter\Filter::html($value));
        //赋值
        $tpl_arr['value'] = $value;
        $tpl_arr['cache'] = $cache;
        //赋值
    ...
    public function display(string $file_name,int $expire=86400)
    {
        ...
        if (defined('NO_VIEW') || !is_file($tpl_c_file) || (filemtime($tpl_c_file) + $expire < time())) {
            ...
            \Core\Base\Template\Template::cache($config);
        }
        return $this;
    }
    ...
    public function static(int $deadtime = 3600)
    {
        ...
        $this->static = true;
        //拼接静态文件路径
        $public_path   = \Core\Framework::$public_path;
        ...
        $root_path = \Core\Framework::$root_path;
        //过期则引入入口文件,重新处理
