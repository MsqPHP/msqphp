...
class Storage
{
	...
	static public function init()
	{
		...
		//载入缓存接口文件
		require CORE_PATH.'Storage/StorageInterface'.EXT;
		//加载驱动
	...
	{
		//驱动路径
		$driver_path = CORE_PATH.'Storage/Driver/'.$type.EXT;
		//文件是否存在
		if(is_file($driver_path) === false) {
			throw new \Exception($type . 'Storage driver 文件无法加载', 500);
		}
		//加载
		require CORE_PATH.'Storage/Driver/'.$type.EXT;
		//拼接类名
		$class_name = 'Core\\Storage\\Driver\\'.$type;
		//创建类
